function [clusts,Quality,D,W] = segment_image(W,R,nGroups,method1,method2,varargin);
%--dr function [mask,clusts,Quality,D,W] = segment_image(IM,R,nGroups,method1,method2,varargin);
...
%--dr [rows,cols,colors]=size(IM);

%%% build distance matrix
%--dr tic; [D,ind_non_zero,rows_nonz,cols_nonz] = imdist(IM,R); ttt=toc;
%--dr disp(['Building affinity matrix took ' num2str(ttt) ' second']);
...
%--dr    for i=1:length(clusts{best_group_index}),
...
%--dr    for i=1:length(clusts{best_group_index}),
%--dr        mask(clusts{best_group_index}{i}) = i;
%--dr    end
else
    error(['WARNING: Unknown method2 ' method2 ' in segment_image']);
end

clusts = clusts{best_group_index};
